...
-- +goose Down
-- Drop only Neno's objects: never DROP SCHEMA public (goose's version table lives there).
DROP TABLE IF EXISTS packs, user_progress, user_answers, user_marks, reviews, users,
  feed_links, feed_items, quiz_options, quiz_questions, course_lessons, courses,
  ss_days, ss_lessons, ss_quarters, hymn_stanzas, hymns, media, hymnals,
  belief_texts, beliefs, egw_paragraphs, egw_editions, egw_books,
  verses, bible_book_names, bible_books, bible_translations, languages;
DROP TYPE IF EXISTS user_role, target_kind, media_kind, feed_kind, content_status;
